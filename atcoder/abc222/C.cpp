...
void Solution() {
    ...
    fo(i, m) {
        // each round
        sort(all(vv), [&](node A, node B) {
            if (A.second.first == B.second.first) return A.second.second < B.second.second;
            return A.second.first > B.second.first;
        });  //custom sort before every round
        for (ll j = 0; j < 2 * n; j += 2) {
            ll whowin = win(vv[j].first[i], vv[j + 1].first[i]);
            // dbg(vv[j].s[i], vv[j + 1].s[i]);
            if (whowin == 1)
                vv[j].second.first += 1;
            else if (whowin == 2)
                vv[j + 1].second.first += 1;
        }
    }
    sort(all(vv), [&](node A, node B) {
        if (A.second.first == B.second.first) return A.second.second < B.second.second;
    ...
    fo(i, 2 * n) cout << vv[i].second.second << '\n';
    // fo(i, 2 * n) cout << vv[i].id << ' ' << vv[i].score << '\n';
}

...
void Solution() {
    ...
    ll n_heroes;
    cin >> n_heroes;
    vector<ll> max_p(n, -INF);  // find max power for given endurance
    for (ll i = 0, p, si; i < n_heroes; ++i) {
        cin >> p >> si, --si;
        max_p[si] = max(max_p[si], p);
    }
    for (ll i = n - 2; i >= 0; --i) max_p[i] = max(max_p[i], max_p[i + 1]);
    Sparse_Table<ll> st(
        a,
    ...
    while (index < n) {
        ...
        while (R > L + 1) {
            int M = (L + R) / 2;
            if (max_p[M - index] >= st.query(index, M)) {
                L = M;
            } else {
                R = M;
            }
